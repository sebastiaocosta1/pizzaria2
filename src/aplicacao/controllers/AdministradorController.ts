...
import InterfaceAdministradorRepository from "../../dominio/repositories/interfaces/InterfaceAdministradorRepository";
...
export default class AdministradorController {
    ...
    async criaAdministrador(req: Request, res: Response): Promise<void> {
        try {
            const { nome, cpf, usuario, senha } = <Administrador>req.body;
            ...
            const novoAdministrador = new Administrador(nome, cpf, usuario, senhaHash);
    ...
    async atualizaAdministrador(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const { nome, cpf, usuario, senha } = req.body as Administrador;
