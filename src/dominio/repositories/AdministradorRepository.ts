...
import { Administrador } from "../entities/AdministradorEntity";
...
export default class AdministradorRepository implements InterfaceAdministradorRepository {
    ...
    async listaAdministrador(id: number): Promise<Administrador> {
        return await this.repository.findOne({ where: { idadministrador: id } });
    }
    ...
    async atualizaAdministrador(
        ...
        try {
            const adminToUpdate = await this.repository.findOne({ where: { idadministrador: id } });
    ...
    async deletaAdministrador(id: number): Promise<{ success: boolean; message?: string }> {
        try {
            const adminToDelete = await this.repository.findOne({ where: { idadministrador: id } });
